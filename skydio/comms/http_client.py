...
class HTTPClient(object):
    ...
    def __init__(self, baseurl, client_id=None, pilot=False, admin=False, token_file=None,
        ...
        self._authenticate(pilot=pilot, admin=admin, token_file=token_file)

    def _authenticate(self, pilot=False, admin=False, token_file=None):
        """ Request an access token from the vehicle. If using a sim, a token_file is required. """
        level = (8 if pilot else 4) if not admin else 12
    ...
    def update_pilot_status(self):
        ...
        self._last_flight_phase = response.get('flightPhase')
        return response

    def get_last_flight_phase(self):
        return self._last_flight_phase

    def prep(self):
        """ Request pre-flight checks. Blocks until ready for takeoff. """
        if self.access_level != 'PILOT':
            fmt_err('Cannot switch to prep: not pilot\n')
            return
    ...
    def set_skill(self, skill_key):
        ...
        res = self.request_json(endpoint, {'args': {}})

        # Set active skill on success, indicated by an empty dict
        print("set_skill {} got response {}".format(skill_key, str(res)))
        if not res:
            print('setting active skill to {}'.format(skill_key))
            self.active_skill = skill_key
